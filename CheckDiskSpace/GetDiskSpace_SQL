...
{		
	...
	$arrServerList = Invoke-Sqlcmd $strQueryServerList -ServerInstance "INSTANCE,1433" -Database "MyDB" -QueryTimeout $intQueryTimeout -ErrorAction 'Stop';	
}		
catch		
{		
	# Set error message	
...
{		
	# Execute query to retrieve server list	
	Invoke-Sqlcmd -Query $queryInsert -ServerInstance "INSTANCE,1433" -Database "MyDB" -QueryTimeout $intQueryTimeout -ErrorAction 'Stop';	
}		
catch		
{		
	# Set error message	
...
Write-Host "Disk space report saved to MyDB Database on INSTANCE"
